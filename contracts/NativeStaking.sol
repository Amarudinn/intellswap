...
contract NativeStaking is IStakingReceiver {
    address public owner;
    uint256 public rewardRate = 100; // 1% per day (100 basis points)
    uint256 public minimumStake = 0.01 ether; // 0.01 native token minimum
    ...
    function stake() external payable {
        require(msg.value >= minimumStake, "Amount below minimum");
    ...
    // Claim rewards (APY + Bonus together)
    ...
    // Get stake info (updated to include bonus)
    ...
    // Emergency withdraw (only unstaked balance)
    function emergencyWithdraw() external onlyOwner {
        uint256 balance = address(this).balance;
        uint256 withdrawable = balance - totalStaked - rewardPool - totalBonusPool;
